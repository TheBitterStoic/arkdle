<template>
  <Layout>
    ...
    <div
      ...
      :class="{ 'bw-mode': isBlackWhiteMode, 'inverted-mode': isInvertedMode }"
    >
      <img
        :src="saddleImagePath"
        alt="Saddle Image"
    ...
    <div class="input-container">
      <template v-if="!isGuessedCorrectly">
        ...
        <ul v-if="filteredSaddles.length > 0 && userInput !== ''" class="dropdown">
          <li v-for="saddle in filteredSaddles" :key="saddle" @click="selectSaddle(saddle)">
            {{ saddle }}
          </li>
        </ul>
      </template>
...
.saddle-input::placeholder {
  color: #bebebe; /* Change the placeholder text color */
